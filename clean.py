	...
	with open("output.csv",'w') as output_csv_file:
		with open("cleaningdata.csv") as input_csv_file:
			...
			for i, row in enumerate(d_reader):
				...
				if doc_input.startswith(('dr ','dra.','dr.')) or doc_input.endswith(("md","md phd","do","d.o.")) or any (c in title_input for c in ("physician","surgeon","surgery","dermatology","orthopedic","gastronenter","cardiologist","md","anesthesiologist","radiology")):
					doc = "Y"
...
def compare_table(row_a,row_b):
	if (row_a["FYE"] < row_b["fye"]):
		...
		if row_a["Org_Name_stand"] < row_b["Org_Name_stand"]:
			return -1
		elif row_a["Org_Name_stand"] == row_b["Org_Name_stand"]:
